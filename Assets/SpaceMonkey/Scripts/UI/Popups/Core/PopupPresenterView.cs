...
namespace SpaceMonkey.Scripts.UI.Popups.Core
{
    ...
    public class PopupPresenterView : MonoBehaviour, IInitializable
    {
        [SerializeField] private Canvas canvas;

        private PopupPresenterService _presenterService;

        public Canvas Canvas => canvas;

        [Inject]
        private void Inject(PopupPresenterService presenterService)
        {
        ...
        {
            Debug.Log($"Show popup : {presenter.name}");
